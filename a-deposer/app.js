...
/* Bips générés par Web Audio API : pas de fichier son à charger, fonctionne
   ...
   iOS suspend le circuit audio dès que l'app passe en arrière-plan (écran
   verrouillé, changement d'appli) et ne le réveille jamais tout seul. On
   retente donc le réveil à CHAQUE bip, pas seulement au lancement — sinon
   seul le tout premier bip d'une séance de chrono s'entend. */
let audioCtx = null;
...
/* Le chrono se recale sur l'horloge réelle à chaque affichage, au lieu de
   décompter les secondes une par une : iOS met en pause le JavaScript
   quand l'app passe en arrière-plan, donc un simple `left--` par tick
   prend du retard sur le temps réel dès qu'on quitte l'écran. En calculant
   `left` depuis l'heure de fin visée, l'affichage est toujours exact dès
   qu'on revient, même après une pause de plusieurs minutes. */
let timerInt = null, timerFin = 0, timerLabel = '', timerBips = null, timerTermine = false;
function startTimer(sec,label){
  stopTimer(false);
  debloquerSon();
  timerFin = Date.now() + sec*1000;
  timerLabel = label; timerBips = new Set(); timerTermine = false;
  $('#timerbar').classList.add('on');
  tickTimer();
  timerInt = setInterval(tickTimer, 250);
}
function tickTimer(){
  const left = Math.max(0, Math.ceil((timerFin - Date.now()) / 1000));
  if(left <= 0){
    if(!timerTermine){
      timerTermine = true;
      ...
      setTimeout(()=>$('#timerbar').classList.remove('on'), 2500);
    }
    clearInterval(timerInt); timerInt=null;
    return;
  }
  $('#timerTxt').textContent = timerLabel+' · '+fmtMMSS(left);
  if(left <= 5 && !timerBips.has(left)){ timerBips.add(left); beep(660, .09, .28); }
}
function stopTimer(){ if(timerInt){ clearInterval(timerInt); timerInt=null; } $('#timerbar').classList.remove('on'); }

/* Rattrapage immédiat au retour au premier plan : sans ça, l'affichage
   reste figé jusqu'au prochain tick (jusqu'à 250 ms) et le son reste
   silencieux tant que rien ne l'a redemandé. */
